<template>
    <div class="config">
        ...
        <p class="tip">Tips: 点击 Update 按钮会将 Openai Key 保存到localStorage中，调用接口时会从 localStorage 中读取 Openai Key
        </p>
        <n-divider />
...
import { OPENAI_KEY, UPDATE_OPENAI_KEY_TIPS, AI_AVATAR, USER_AVATAR } from '../constants'
...
const onChange = async (type: number, options: any, e: InputEvent) => {
    ...
    try {
        ...
        type === 1 ? currentUserAvatar.value = result : currentAIAvatar.value = result
